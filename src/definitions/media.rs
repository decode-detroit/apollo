...
pub struct MediaCue {
    ...
    pub channel: ChannelNumber, // the channel of the video or audio. New media sent to the same channel will replace the old media, starting instantly
...
/// A struct to hold the dimensions of a video frame (with a window)
///
#[derive(Clone, PartialEq, Eq, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct VideoFrameWithWindow {
    pub window_number: WindowNumber, // the application window number for the channel
    pub top: i32,                    // the distance (in pixels) from the top of the display
    pub left: i32,                   // the distance (in pixels) from the left of the display
    pub height: i32,                 // the height of the video
    pub width: i32,                  // the width of the video
...
pub struct MediaChannel {
    pub channel: ChannelNumber,                    // the channel number
    pub video_frame: Option<VideoFrameWithWindow>, // the video frame. Defaults to a new application window generated by gstreamer
...
pub struct ChannelState {
    pub channel: ChannelNumber, // the channel of the video or audio
...
pub struct WindowDefinition {
    pub window_number: WindowNumber, // the unique identifier for the window
...
/// A type to uniquely store channel numbers
///
pub type ChannelNumber = u32;

/// A type to uniquely store window numbers
///
pub type WindowNumber = u16;
...
pub struct VideoStream {
    pub channel: ChannelNumber, // the channel where the video should be played
    pub window_number: WindowNumber, // the window where the video should be played
    pub allocation: gdk4::Rectangle, // the location of the video in the screen
    pub video_sink: gst::Element, // the video widget which will display the video
}
...
/// An enum to define the four directions for channel alignment
#[derive(PartialEq, Eq, Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub enum Direction {
    // Variants in all four directions
    Up,
    Down,
    Left,
    Right,
...
/// A struct to define a realignment for a video channel
///
#[derive(Clone, PartialEq, Eq, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ChannelRealignment {
    pub channel: ChannelNumber, // the channel number
    pub direction: Direction,   // the direction of the change
}
